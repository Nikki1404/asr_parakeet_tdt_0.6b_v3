...
==================================================
Modified to follow these rules:

1) Baseline transcript is NOT ground truth.
   - It is only a reference point.
   - Each stage is evaluated independently for quality.

2) Do NOT rewrite spoken sentence.
   - No paraphrasing.
   - No grammar rewriting of user intent.
   - No sentence restructuring.

3) Allowed normalization is ONLY for scoring / analysis:
   - punctuation quality checks
   - spelling/domain correction hints
   - context-aware numeric interpretation
   - VAD/truncation quality checks
   - short-word confusion analysis

4) Numeric handling is context-aware.
   - Avoid wrong conversions like: "to" -> 2
   - Only interpret digit words as numeric when nearby context strongly suggests it.

5) Comparison logic now uses:
   - quality scoring
   - numeric quality
   - punctuation quality
   - VAD quality
   - domain phrase quality
   - transcript similarity only as a secondary indicator
...
STAGES = {
    0: {
        ...
        "outcome": "Reference point for comparison",
    },
    1: {
        "name": "asr_config",
        "phase": "Setup",
        "task": "ASR Config Finalization",
        "description": "Lock language candidates, fixed WAV telephony-style input format",
    ...
    3: {
        ...
        "description": "Boost digits, identifiers, and domain-specific phrases",
    ...
    4: {
        ...
        "description": "Mine repeated useful terms from earlier transcripts and boost them",
    ...
    5: {
        ...
        "description": "Analyze ITN/Lexical/Display and context-aware numeric interpretation",
    ...
    6: {
        ...
        "phase": "Accuracy",
        "task": "Dictation Mode",
        "description": "Enable natural punctuation handling",
        "outcome": "More readable transcript",
    },
    7: {
        "name": "emotion_tone",
        "phase": "Quality",
        "task": "Emotion / Tone Evaluation",
        "description": "Confidence/rate/disfluency proxy analysis",
        "outcome": "Robust quality measurement under varied speech",
    },
    8: {
    ...
    9: {
        ...
        "description": "Push stream / chunk-based ingestion",
    ...
    10: {
        ...
        "description": "Concurrent stream stability / throttling checks",
...
def extract_digit_like_sequences(text: str):
    """
    Identify digit-like sequences for analysis only.
    Does NOT rewrite the original transcript.
    Avoids converting ambiguous short words unless context is numeric.
    """
...
def evaluate_vad_quality(text: str, segments: list) -> dict:
    ...
    for seg in segments:
        ...
        if prev and s and prev and (s.startswith(prev[: max(3, min(len(prev), 10))]) or prev.startswith(s[: max(3, min(len(s), 10))])):
            repeated_prefixes += 1
        prev = s
...
def score_transcript_quality(text: str, segments: list) -> dict:
    ...
    overall = round(
        punctuation["score"] * 0.18 +
        numeric["score"] * 0.28 +
        domain["score"] * 0.12 +
        vad["score"] * 0.18 +
        short_words["score"] * 0.12 +
...
def save_stage_files(stage_num: int, stage_meta: dict, result: dict):
    ...
    with open(os.path.join(stage_dir, "metrics_summary.txt"), "w", encoding="utf-8") as f:
        ...
        lines += [
            ...
            f"  punctuation_quality_score : {result.get('quality_scores', {}).get('punctuation_quality', {}).get('score')}",
            f"  numeric_quality_score     : {result.get('quality_scores', {}).get('numeric_quality', {}).get('score')}",
            f"  domain_quality_score      : {result.get('quality_scores', {}).get('domain_quality', {}).get('score')}",
            f"  vad_quality_score         : {result.get('quality_scores', {}).get('vad_quality', {}).get('score')}",
            f"  short_word_quality_score  : {result.get('quality_scores', {}).get('short_word_quality', {}).get('score')}",
            f"  readability_quality_score : {result.get('quality_scores', {}).get('readability_quality', {}).get('score')}",
            "",
            "── Transcript ──────────────────────",
...
def print_comparison(comp: dict):
    ...
    for metric, d in comp["metric_deltas"].items():
        ...
        print(f"  {metric:<26}: {d['prev']} → {d['curr']}  ({sign}{d['change']}) {sym} {d['direction']}")
...
def print_full_table(report: dict):
    ...
    print(f"{'=' * 90}")
    print(f"  {'#':<3} {'Stage':<22} {'Phase':<14} {'Words':>6} {'Segs':>5} {'Qual':>7} {'Conf':>7} {'TTFT-F':>8}")
    print(f"  {'─' * 3} {'─' * 22} {'─' * 14} {'─' * 6} {'─' * 5} {'─' * 7} {'─' * 7} {'─' * 8}")
    ...
    if len(stages) >= 2:
        ...
        for metric in ["overall_quality_score", "word_count", "avg_confidence", "ttft_final_ms"]:
            bv = bm.get(metric)
            lv = lm.get(metric)
            ...
            diff = round(lv - bv, 4)
            sign = "+" if diff > 0 else ""
            print(f"    {metric:<24}: {bv} → {lv}  ({sign}{diff})")
